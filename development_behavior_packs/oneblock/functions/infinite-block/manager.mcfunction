...
execute unless block ~ ~ ~ minecraft:chest run kill @e[type=minecraft:marker,tag=ija-a4-chest]
function ija-one-block:infinite-block/show-particles
...
execute as @a[x=-2,dx=4,y=60,dy=2,z=-2,dz=4] at @s if block ~ ~ ~ minecraft:barrier run tp @s ~ ~.3 ~
execute if block ~ ~ ~ minecraft:barrier run scoreboard players add @s ija-a4-persistent-barrier-counter 1
execute unless block ~ ~ ~ minecraft:barrier run scoreboard players set @s ija-a4-persistent-barrier-counter 0
execute if entity @s[scores={ija-a4-persistent-barrier-counter=20..}] run setblock ~ ~ ~ minecraft:dirt{}
